...
extern int noisy, NFunCall, NEigenQ, NPMatUVRoot, *ancestor, GeneticCode[][64],GradientB_NFunCall;
...
*/
      ...
      if(iteration && np) {
         if(com.method == 1)
            ...
            j = minB2(noisy>2?frub:NULL, &lnL,x,xb, e, com.space);
             ...
             j = fake_ming2(noisy>2?frub:NULL,&lnL,CUDA_lfundG,NULL,x,xb, com.space,e,np, com.ntime);
#else
      ...
      printf("%d lfun, %d eigenQcodon, %d P(t), %d GradientB_call_lfun\n",NFunCall, NEigenQ, NPMatUVRoot, GradientB_NFunCall);
      if (itree==0)
...
*/
   ...
   *S = 0;
   ...
   for(iw=0; iw<com.ncatG; iw++) {  /* com.ncatG has the number of w values for each site */
      ...
      //CUDA_ScheduleConditionalPNode(NULL, 0, x, -3);
      for(h=0; h<com.npatt; h++) {
