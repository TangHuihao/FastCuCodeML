...
double extendUVR[3 * UVR_SIZE];                  /* extend U,V to 64*64 and R to 64 */      
...
double *dev_fake_com_conP[MAXCARD];
...
//thh_edit
//void fake_schedule(int dev_id, int pos0, int pos1, int get_parameters);
//void getfakecallKernelConditionalPNodeInternal(int dev_id, int pos0, int pos1, int inode, int ison);
...
void callKernelConditionalPNodeTipClean(int dev_id, int pos0, int pos1, int inode, int ison, int get_parameters)
{
/*
...
*/
    ...
    //if (get_parameters == -2) {
    //    cutilSafeCall(cudaMemcpy(dev_fake_com_conP[dev_id] + global_ir * inode * 64 * npatt_extend + inode * 64 * npatt_extend, dev_com_conP[dev_id] + inode * 64 * npatt_extend, 64 * npatt_extend * sizeof(double), cudaMemcpyDeviceToDevice));
    //    //cutilSafeCall(cudaMemcpy(fake_com_conP + global_ir * inode * 64 * npatt_extend + inode * 64 * npatt_extend, dev_com_conP[dev_id] + inode * 64 * npatt_extend, 64 * npatt_extend * sizeof(double), cudaMemcpyDeviceToHost));
    //    cutilCheckMsg("cudaMemcpy dev_fake_com_conP to dev_com_conP failed");
    //}
}

void fakecallKernelConditionalPNodeTipClean(int dev_id, int pos0, int pos1, int inode, int ison, int get_parameters)
{
    /*
      Call one of the above two kernels according to whether the son node(ison) is the first son or not.
      If STREAM is defined, use the stream to provide more parallelism.
    */
    ...
    if (inodes_modified == NULL) {
        ModifyOrNot = false;
}
    else {
        for (int i = 0; i < tree.nnode; i++) {
            if (inodes_modified[i] == ison) {
                ModifyOrNot = true;
                break;
            }
            else if (inodes_modified[i] == -1)
                break;
        }
    }
    if (ModifyOrNot) {
    ...
    //else {
    //    //dev_com_conP[dev_id][inode * 64 * npatt_extend] = *dev_fake_com_conP[dev_id] + (size_t)(global_ir * tree.nnode * 64 * npatt_extend + inode * 64 * npatt_extend);
    //    //cutilSafeCall(cudaMemcpy(dev_com_conP[dev_id] + inode * 64 * npatt_extend, dev_fake_com_conP[dev_id] + global_ir * tree.nnode * 64 * npatt_extend + inode * 64 * npatt_extend, 64 * npatt_extend * sizeof(double), cudaMemcpyDeviceToDevice));
    //    //cutilSafeCall(cudaMemcpy(fake_com_conP + global_ir * tree.nnode * 64 * npatt_extend + inode * 64 * npatt_extend, dev_fake_com_conP[dev_id] + global_ir * tree.nnode * 64 * npatt_extend + inode * 64 * npatt_extend, 64 * npatt_extend * sizeof(double), cudaMemcpyDeviceToHost));
    //    //cutilCheckMsg("cudaMemcpy dev_fake_com_conP to dev_com_conP failed");
    //}
...
void callKernelConditionalPNodeTipUnclean(int dev_id, int pos0, int pos1, int inode, int ison, int get_parameters)
{
    int threadsPerBlock = com.ncode;
    int blocksPerGrid = pos1 - pos0;
    if (first_son[ison]) {
...
#if __CUDA_ARCH__ >= 35               //thh_edit(350-35)        /* if arch>=35, using __ldg() to load data from global memory */
__global__
...
//      Kernel of calculating exptRootAll[]=exp(t*UVR[]);
//    */
//    __shared__ double sUVR[64 * 64 * 2];
//
//    int inode = blockIdx.x;
//    if (inode == DEV_tree_root) return;
//
...
//    int ridx = PMatiUVR[inode] * UVR_SIZE + 64 * 64 * 2 + tx;
//
//    // Load UVR into shared memory
//    for (int i = tx; i < 64 * 64 * 2; i += blockDim.x) {
//        sUVR[i] = UVR[i];
//    }
//
//    __syncthreads();
//
//    double uvr = sUVR[ridx];
//
//    // Unroll loop
//    exptRootAll[idx] = exp(t * uvr);
//    exptRootAll[idx + 1] = exp(t * sUVR[ridx + 1]);
//    exptRootAll[idx + 2] = exp(t * sUVR[ridx + 2]);
//    // ...
//}

//thh_edited 

//share memory version // when the tree.node is too big the shared memory may cause error how to solve it ??
__global__
void kernelPMatExptRoot(const double* PMatT, const int* PMatiUVR, const double* UVR, double* exptRootAll)
{
   /* int inode = blockIdx.x;
    if (inode == DEV_tree_root) return;*/
    __shared__ int inode_shared;
    inode_shared = blockIdx.x;
    if (inode_shared == DEV_tree_root) return;

    __shared__ double UVR_shared[64];
    __shared__ double t_shared;
    __shared__ int idx_shared[64];

    // Load UVR into shared memory
    //int ridx = PMatiUVR[inode] * UVR_SIZE + 64 * 64 * 2 + threadIdx.x;
    UVR_shared[threadIdx.x] = UVR[PMatiUVR[inode_shared] * UVR_SIZE + 64 * 64 * 2 + threadIdx.x];
    
    //double t = PMatT[inode];
    t_shared = PMatT[inode_shared];


    __syncthreads();
    
    double exp_t_UVR = exp(t_shared * UVR_shared[threadIdx.x]);
    

    // Compute exponential function on a per-warp basis
//#pragma unroll
//    for (int i = 32; i >= 1; i /= 2) {
//        exp_t_UVR += __shfl_xor_sync(0xffffffff, exp_t_UVR, i, 64);
//    }
    //int idx = inode * 64 + threadIdx.x;
    idx_shared[threadIdx.x] = inode_shared * 64 + threadIdx.x;


    exptRootAll[idx_shared[threadIdx.x]] = exp_t_UVR;
}
//register _version
//__global__
//void kernelPMatExptRoot(const double* PMatT, const int* PMatiUVR, const double* UVR, double* exptRootAll)
...
//    double t = PMatT[inode];
//    int ridx = PMatiUVR[inode] * UVR_SIZE + 64 * 64 * 2 + tx;
//    double exptRoot = exp(t * UVR[ridx]);
//
//    // Use registers instead of shared memory
//    double b0 = UVR[ridx];
//    double b1 = UVR[ridx + 64];
//    double b2 = UVR[ridx + 128];
//    double b3 = UVR[ridx + 192];
//    double b4 = UVR[ridx + 256];
//    double b5 = UVR[ridx + 320];
//    double b6 = UVR[ridx + 384];
//    double b7 = UVR[ridx + 448];
//    double b8 = UVR[ridx + 512];
//    double b9 = UVR[ridx + 576];
//    double b10 = UVR[ridx + 640];
//    double b11 = UVR[ridx + 704];
//    double b12 = UVR[ridx + 768];
//    double b13 = UVR[ridx + 832];
//    double b14 = UVR[ridx + 896];
//    double b15 = UVR[ridx + 960];
//
//    exptRootAll[idx] = exptRoot;
//}
...
//thh_edited 
//__global__ void kernelPMatUVRoot(const int* __restrict__ PMatiUVR,
//    const double* __restrict__ UVR,
//    const double* __restrict__ exptRootAll,
//    double* __restrict__ PMat)
//{
//    const int inode = blockIdx.y;
//    if (inode == DEV_tree_root) return;
//
...
//    const int ibx = blockIdx.x * m;
//
//    const double* __restrict__ U = UVR + PMatiUVR[inode] * UVR_SIZE;
//    const double* __restrict__ V = U + n * n;
//    double* __restrict__ P = PMat + inode * n * n;
//
//    U += ibx * n;
//    V += id;
//    P += id * n + ibx;
...
//        const int j = i + id % warpSize;
//        bs[id / warpSize + j][id % warpSize] = U[j * n + id] * exptRoot;
//    }
//    __syncthreads();
//
//    double c[m] = { 0 };
...
//        V += n;
//        v1 = V[0];
//        for (int j = 0; j < m; j += warpSize) {
//            const int k = j + id % warpSize;
//            if (k < m) c[k] += v0 * bs[i][k];
//        }
//    }
//
//    for (int i = 0; i < m; i++) {
...
void callKernelPMatUVRootAll(int dev_id)
{
    ...
    /*if (tree.nnode * com.ncode * sizeof(double) > 49000) {
        printf("\n\ncallKernelPMatUVRootAll share memory big!\n");
        exit(0);
    }*/
    //kernelPMatExptRoot<<<tree.nnode, com.ncode, tree.nnode * com.ncode * sizeof(double) >>>(dev_PMatT[dev_id], dev_PMatiUVR[dev_id], dev_UVR[dev_id], dev_exptRootAll[dev_id]);
    cutilCheckMsg("cudaKernel kernelPMatExptRoot failed");
    //thh_edit
   /* double *thh_exptRootAll = (double*)malloc(tree.nnode*com.ncode*sizeof(double)) ;
    cutilSafeCall(cudaMemcpy(thh_exptRootAll, dev_exptRootAll[dev_id], tree.nnode * com.ncode * sizeof(double), cudaMemcpyDeviceToHost));
    cutilCheckMsg("cudaMemcpy dev_PMatiUVR failed");*/
...
*/
    ...
    cutilSafeCall(cudaMalloc((void**)&dev_fake_com_conP[dev_id], com.ncatG * tree.nnode * 64 * npatt_extend * sizeof(double)));
    cutilCheckMsg("cudaMalloc dev_fake_com_conP failed");
    cutilSafeCall(cudaMemset(dev_fake_com_conP[dev_id], 0, com.ncatG * tree.nnode * 64 * npatt_extend * sizeof(double)));
    cutilCheckMsg("cudaMalloc dev_com_conP failed");
    //(Malloc((void**)&dev_fake_com_conP[dev_id], com.ncatG * tree.nnode * 64 * npatt_extend * sizeof(double)));
...
long long posTest(int ig, double *x, int count)
{
    ...
    while (count--) {
        CUDA_ScheduleConditionalPNode(0, ig, x, -3);
    }
...
*/
    ...
    cutilSafeCall(cudaFree(dev_fake_com_conP[dev_id]));
    cutilCheckMsg("cudaFree dev_fake_com_conP failed");
...
/// <summary>
/// thh_edit remove extern for next 4 line
/// </summary>
...
*/  
    /*double thh_arr[] = {1,2,3,4,5};
    double* thh_UVR = thh_arr + 2;*/
    ...
    for (i = 0; i < 3; i++) {
        ...
        for (j = 0; j < com.ncode; j++) {
            /*double* thh_extendUVR = extendUVR + i * UVR_SIZE + j * 64;
            double* thh_UVR = UVR + i * UVR_SIZE + j * com.ncode;
            double thh_n = com.ncode * sizeof(double);*/
...
void fakecallKernelConditionalPNodeInternal(int dev_id, int pos0, int pos1, int inode, int ison)
{
    int bly;
    if ((pos1 - pos0) % 32 == 0) {
        bly = (pos1 - pos0) / 32;
    ...
    //else {     
    //    //cutilSafeCall(cudaMemcpy(dev_com_conP[dev_id] + inode * 64 * npatt_extend , dev_fake_com_conP[dev_id] + global_ir * tree.nnode * 64 * npatt_extend + inode * 64 * npatt_extend, 64 * npatt_extend * sizeof(double), cudaMemcpyDeviceToDevice));
    //    //cutilSafeCall(cudaMemcpy(fake_com_conP + global_ir * tree.nnode * 64 * npatt_extend + inode * 64 * npatt_extend, dev_com_conP[dev_id] + inode * 64 * npatt_extend, 64 * npatt_extend * sizeof(double), cudaMemcpyDeviceToHost));
    //    //cutilCheckMsg("cudaMemcpy dev_fake_com_conP to dev_com_conP failed");
    //}
}

void callKernelConditionalPNodeInternal(int dev_id, int pos0, int pos1, int inode, int ison, int get_parameters)
{
    int bly;
    if ((pos1 - pos0) % 32 == 0) {
        bly = (pos1 - pos0) / 32;
...
#else
void schedule(int dev_id, int pos0, int pos1, int get_parameters)
#endif
{
/*
...
*/
    ...
    if (get_parameters > -2) {
        /*for (int i = 0; i < tree.nnode; i++) 
            if (inodes_modified[i] > -1) {
            cutilSafeCall(cudaMemcpy(dev_com_conP[dev_id] + inodes_modified[i] * 64 * npatt_extend, dev_fake_com_conP[dev_id] + global_ir * tree.nnode * 64 * npatt_extend + inodes_modified[i] * 64 * npatt_extend, 64 * npatt_extend * sizeof(double), cudaMemcpyDeviceToDevice));
            cutilCheckMsg("cudaMemcpy dev_fake_com_conP to dev_com_conP failed");
        }*/
        cutilSafeCall(cudaMemcpy(dev_com_conP[dev_id], dev_fake_com_conP[dev_id] + global_ir * tree.nnode * 64 * npatt_extend, tree.nnode * 64 * npatt_extend * sizeof(double), cudaMemcpyDeviceToDevice));
        cutilCheckMsg("cudaMemcpy dev_fake_com_conP to dev_com_conP failed");
    }
    int i;
    for (i = 0; i < tree.nnode + com.NnodeScale - 1; i++) {
        if (get_parameters == -2 || get_parameters == -3) {
            ...
            if (schedule_scale[i]) {                        /* the current node needs scaling */
                ...
                callKernelConditionalPNodeTipClean(dev_id, pos0, pos1, father[inode], inode, get_parameters);
            }
            else if (nodes[inode].nson < 1 && !com.cleandata) {
                callKernelConditionalPNodeTipUnclean(dev_id, pos0, pos1, father[inode], inode, get_parameters);
            }
            else {             /* internal */
#ifdef STREAM
...
#endif  
                callKernelConditionalPNodeInternal(dev_id, pos0, pos1, father[inode], inode, get_parameters);

            }
            if (get_parameters == -2) {
                cutilSafeCall(cudaMemcpy(dev_fake_com_conP[dev_id] + global_ir * tree.nnode * 64 * npatt_extend + father[inode] * 64 * npatt_extend, dev_com_conP[dev_id] + father[inode] * 64 * npatt_extend, 64 * npatt_extend * sizeof(double), cudaMemcpyDeviceToDevice));
                cutilCheckMsg("cudaMemcpy dev_fake_com_conP to dev_com_conP failed");
            }
        }
        else {
            ...
            if (schedule_scale[i]) {                        /* the current node needs scaling */
                ...
                fakecallKernelConditionalPNodeTipClean(dev_id, pos0, pos1, father[inode], inode, get_parameters);
            }
            else if (nodes[inode].nson < 1 && !com.cleandata) {
                callKernelConditionalPNodeTipUnclean(dev_id, pos0, pos1, father[inode], inode, get_parameters);
            }
                ...
                cutilCheckMsg("cudaStreamSynchronize failed");
#endif  
                fakecallKernelConditionalPNodeInternal(dev_id, pos0, pos1, father[inode], inode);
            }
        }
        //cutilSafeCall(cudaDeviceSynchronize());
        //cutilCheckMsg("cudaDeviceSynchronize failed");
...
void CUDA_ScheduleConditionalPNode(int ir, int ig, double *x, int get_parameters)
{
/*
...
*/
    ...
    schedule(SINGLE_GPU_ID, pos0, pos1, get_parameters);
#endif                   /* HYBRID */
#endif                   /* MULTICARD_ONLY_GPU */
}
